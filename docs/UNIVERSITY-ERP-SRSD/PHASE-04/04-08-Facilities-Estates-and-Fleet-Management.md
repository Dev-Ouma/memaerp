# MOD-04-08: Campus Facilities, Maintenance & Fleet Bookings — SOFTWARE REQUIREMENTS SPECIFICATION

- **Module Identifier:** `MOD-04-08`
...
- **Module ID:** `MOD-04-08`
- **Official Name:** Campus Facilities, Maintenance & Fleet Bookings
...
Manages physical university infrastructure and logistics: building and space inventory, room reservation scheduling (non-academic events), preventative and corrective maintenance work orders, fleet management, vehicle bookings, fuel consumption, and driver logs.

## 4. Scope
### 4.1 In-Scope
- Campus building, floor, room, and outdoor facility master registry
- Event space and conference room reservation with approval workflows
- Facility maintenance work order ticketing and technician dispatch
- Vehicle fleet register, insurance tracking, and roadworthiness inspection logs
- Official university transport and vehicle booking requests
- Driver assignment, mileage logging, and fuel card consumption tracking
...
| **Domain Coordinator / Officer** | Operates day-to-day estates & fleet management workflows, processes applications, and verifies compliance. | Staff |
...
### FR-08-001: Core Campus Facilities, Maintenance & Fleet Bookings Management
- **Description:** System shall manage the end-to-end lifecycle for campus facilities, maintenance & fleet bookings with full compliance and state tracking.
...
### FR-08-002: Campus Facilities, Maintenance & Fleet Bookings Reporting and Auditing
...
|---|---|---|
| **SUB-08-01** | Intake & Registration Hub | Handles intake, verification, and initial processing of campus facilities, maintenance & fleet bookings items. |
| **SUB-08-02** | Review & Evaluation Engine | Manages rubrics, peer reviews, committee evaluations, and scoring. |
| **SUB-08-03** | Milestone & Compliance Tracker | Tracks progress milestones, renewals, SLA deadlines, and closures. |
...
- **BR-MOD-04-08-001 (Conflict of Interest Prevention):** Reviewers and committee members cannot evaluate submissions where they are named investigators, supervisors, or interested parties.
- **BR-MOD-04-08-002 (Document Immutability):** Once approved by committee or authority, final signed documents and certificates cannot be modified without formal amendment workflow.
- **BR-MOD-04-08-003 (Audit Trail Completeness):** Every evaluation score, status update, note, and decision must capture user ID, timestamp, IP address, and previous state.

## 13. Data Requirements & Entities
### Core PostgreSQL Relational Tables:
#### Table: `mod_04_08.primary_records`
*Description: Master data registry for Campus Facilities, Maintenance & Fleet Bookings.*
```sql
-- Schema defined in detailed implementation phase
-- Core entity for Campus Facilities, Maintenance & Fleet Bookings
...
## 15. Validation Rules
- **VR-MOD-04-08-001 [tracking_number]:** Must follow institutional numbering format e.g. UNIV/MOD/YYYY/XXXXX.
...
- **Campus Facilities, Maintenance & Fleet Bookings Executive Dashboard (Deans, Directors, Executive Committee):** Real-time pipeline volume, approval velocity, overdue items, compliance indicators, and strategic KPIs for campus facilities, maintenance & fleet bookings.

## 19. Reports
| Report ID | Title | Frequency | Output Formats | Permitted Roles |
|---|---|---|---|---|
| `REP-08-01` | Campus Facilities, Maintenance & Fleet Bookings Master Register | Monthly / On-Demand | PDF, Excel, CSV | Management, Auditors, Regulators |
| `REP-08-02` | Campus Facilities, Maintenance & Fleet Bookings Performance & SLA Report | Quarterly | PDF, CSV | Quality Assurance, Senate |
...
GET  /api/v1/04/08
POST /api/v1/04/08
GET  /api/v1/04/08/reports
GET  /api/v1/04/08/dashboard
```
...
- **Inbound Dependencies (Prerequisites):** MOD-01-02 (Campuses/Buildings), MOD-03-01 (Staff Requesters)
- **Outbound Dependencies (Consuming Modules):** MOD-03-08 (Fleet & Maintenance Invoicing), MOD-03-11 (Spare Parts Stores).

## 25. System-Generated Documents
- **Campus Facilities, Maintenance & Fleet Bookings Official Documentation:** Format `PDF`. Standardized output certificates, transcripts, notices, and approval summaries for campus facilities, maintenance & fleet bookings.
...
|---|---|---|---|
| `ERR-08-001` | `400 Bad Request` | Missing mandatory documentation or invalid parameters | Highlight missing checklist items in UI. |
| `ERR-08-002` | `403 Forbidden` | Reviewer conflict of interest or unauthorized status change | Block action and log security warning in audit log. |
...
- [ ] **AC-1:** Complete lifecycle workflow for campus facilities, maintenance & fleet bookings executes without data loss.
...
|---|---|---|---|
| `TC-08-01` | End-to-End Campus Facilities, Maintenance & Fleet Bookings Lifecycle | 1. Submit initial dossier. 2. Pass administrative screening. 3. Complete review cycle. 4. Record committee decision. 5. Verify certificate generation. | Submission progresses smoothly across stages and outputs valid signed documentation. |
