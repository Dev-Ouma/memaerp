# MOD-03-03: Leave Types, Balances, Approvals & Calendar — SOFTWARE REQUIREMENTS SPECIFICATION

- **Module Identifier:** `MOD-03-03`
...
- **Module ID:** `MOD-03-03`
- **Official Name:** Leave Types, Balances, Approvals & Calendar
...
Manages staff leave lifecycle: leave type configuration (annual, sick, maternity, study, sabbatical), balance accrual, application submission, multi-tier approval, and team calendar integration.

## 4. Scope
### 4.1 In-Scope
- Configurable leave types with accrual rules
- Leave balance calculation and carry-forward policies
- Self-service leave application with date picker
- Multi-tier approval chain (HOD -> HR Director)
- Team leave calendar showing departmental availability
...
### FR-03-001: Core Leave Types, Balances, Approvals & Calendar Operations
- **Description:** System shall support end-to-end leave types, balances, approvals & calendar operations with ACID-compliant transaction integrity.
...
### FR-03-002: Leave Types, Balances, Approvals & Calendar Reporting Suite
- **Description:** System shall generate statutory, management, and operational reports for leave types, balances, approvals & calendar.
...
|---|---|---|
| **SUB-03-01** | Core Transaction Engine | Primary transactional operations for leave types, balances, approvals & calendar. |
| **SUB-03-02** | Approval & Workflow Manager | Configurable multi-tier approval workflows. |
| **SUB-03-03** | Reporting & Analytics | Dashboards, scheduled reports, and ad-hoc queries. |
...
- **BR-MOD-03-03-001 (Separation of Duties):** The same user cannot both initiate and approve a financial transaction.
- **BR-MOD-03-03-002 (Budget Check):** All expenditure transactions must pass budget availability validation before approval.
- **BR-MOD-03-03-003 (Audit Compliance):** All state changes produce immutable audit records with previous/new values.

## 13. Data Requirements & Entities
### Core PostgreSQL Relational Tables:
#### Table: `mod_03_03.primary_records`
*Description: Primary data table for Leave Types, Balances, Approvals & Calendar.*
```sql
-- Schema defined in detailed implementation phase
-- Core entity for Leave Types, Balances, Approvals & Calendar
...
## 15. Validation Rules
- **VR-MOD-03-03-001 [status]:** Must be a valid workflow state value per module state machine.
...
- **Leave Types, Balances, Approvals & Calendar Dashboard (Module Administrator / Management):** Operational metrics, KPIs, trend charts, and exception alerts for leave types, balances, approvals & calendar.

## 19. Reports
| Report ID | Title | Frequency | Output Formats | Permitted Roles |
|---|---|---|---|---|
| `REP-03-01` | Leave Types, Balances, Approvals & Calendar Summary Report | Monthly / On-Demand | PDF, Excel, CSV | Management, Finance, HR |
| `REP-03-02` | Leave Types, Balances, Approvals & Calendar Detailed Transaction Log | Daily / On-Demand | CSV, Excel | Auditors, Officers |

## 20. Search, Filtering and Export Requirements
- **Search Capabilities:** Full-text search across leave types, balances, approvals & calendar records.
...
GET  /api/v1/03/03
POST /api/v1/03/03
GET  /api/v1/03/03/reports
GET  /api/v1/03/03/dashboard
```
...
- **Inbound Dependencies (Prerequisites):** MOD-03-01 (Employee data)
- **Outbound Dependencies (Consuming Modules):** MOD-03-06 (Leave deductions/encashment).

## 25. System-Generated Documents
- **Leave Types, Balances, Approvals & Calendar Report:** Format `PDF`. Official generated report for leave types, balances, approvals & calendar.
...
|---|---|---|---|
| `ERR-03-001` | `400 Bad Request` | Invalid input data or business rule violation | Return detailed validation error messages. |
| `ERR-03-002` | `409 Conflict` | Budget exceeded or duplicate transaction detected | Reject with explanation and suggest corrective action. |
...
- [ ] **AC-1:** End-to-end leave types, balances, approvals & calendar workflow operates correctly with ACID integrity.
...
|---|---|---|---|
| `TC-03-01` | Core Leave Types, Balances, Approvals & Calendar Workflow | 1. Initiate transaction. 2. Route through approval chain. 3. Execute. 4. Verify audit trail. | Transaction processed, approvals recorded, audit trail complete. |
