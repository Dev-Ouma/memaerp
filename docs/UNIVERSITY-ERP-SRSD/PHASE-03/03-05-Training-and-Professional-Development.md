# MOD-03-05: Training Needs, Calendar & CPD Tracking — SOFTWARE REQUIREMENTS SPECIFICATION

- **Module Identifier:** `MOD-03-05`
...
- **Module ID:** `MOD-03-05`
- **Official Name:** Training Needs, Calendar & CPD Tracking
...
Manages staff training and continuing professional development: training needs assessment, course catalogue, training calendar, enrollment, budget tracking, and CPD hour logging.

## 4. Scope
### 4.1 In-Scope
- Training needs assessment (TNA) workflows
- Internal and external training course catalogue
- Training calendar and enrollment management
- Training budget allocation and expenditure tracking
- Post-training evaluation and ROI measurement
- CPD hour logging and certification tracking
...
### FR-05-001: Core Training Needs, Calendar & CPD Tracking Operations
- **Description:** System shall support end-to-end training needs, calendar & cpd tracking operations with ACID-compliant transaction integrity.
...
### FR-05-002: Training Needs, Calendar & CPD Tracking Reporting Suite
- **Description:** System shall generate statutory, management, and operational reports for training needs, calendar & cpd tracking.
...
|---|---|---|
| **SUB-05-01** | Core Transaction Engine | Primary transactional operations for training needs, calendar & cpd tracking. |
| **SUB-05-02** | Approval & Workflow Manager | Configurable multi-tier approval workflows. |
| **SUB-05-03** | Reporting & Analytics | Dashboards, scheduled reports, and ad-hoc queries. |
...
- **BR-MOD-03-05-001 (Separation of Duties):** The same user cannot both initiate and approve a financial transaction.
- **BR-MOD-03-05-002 (Budget Check):** All expenditure transactions must pass budget availability validation before approval.
- **BR-MOD-03-05-003 (Audit Compliance):** All state changes produce immutable audit records with previous/new values.

## 13. Data Requirements & Entities
### Core PostgreSQL Relational Tables:
#### Table: `mod_03_05.primary_records`
*Description: Primary data table for Training Needs, Calendar & CPD Tracking.*
```sql
-- Schema defined in detailed implementation phase
-- Core entity for Training Needs, Calendar & CPD Tracking
...
## 15. Validation Rules
- **VR-MOD-03-05-001 [status]:** Must be a valid workflow state value per module state machine.
...
- **Training Needs, Calendar & CPD Tracking Dashboard (Module Administrator / Management):** Operational metrics, KPIs, trend charts, and exception alerts for training needs, calendar & cpd tracking.

## 19. Reports
| Report ID | Title | Frequency | Output Formats | Permitted Roles |
|---|---|---|---|---|
| `REP-05-01` | Training Needs, Calendar & CPD Tracking Summary Report | Monthly / On-Demand | PDF, Excel, CSV | Management, Finance, HR |
| `REP-05-02` | Training Needs, Calendar & CPD Tracking Detailed Transaction Log | Daily / On-Demand | CSV, Excel | Auditors, Officers |

## 20. Search, Filtering and Export Requirements
- **Search Capabilities:** Full-text search across training needs, calendar & cpd tracking records.
...
GET  /api/v1/03/05
POST /api/v1/03/05
GET  /api/v1/03/05/reports
GET  /api/v1/03/05/dashboard
```
...
- **Inbound Dependencies (Prerequisites):** MOD-03-01, MOD-03-04 (Skill gaps from appraisals)
- **Outbound Dependencies (Consuming Modules):** MOD-03-06 (Training allowances), MOD-04-05 (Budget allocation).

## 25. System-Generated Documents
- **Training Needs, Calendar & CPD Tracking Report:** Format `PDF`. Official generated report for training needs, calendar & cpd tracking.
...
|---|---|---|---|
| `ERR-05-001` | `400 Bad Request` | Invalid input data or business rule violation | Return detailed validation error messages. |
| `ERR-05-002` | `409 Conflict` | Budget exceeded or duplicate transaction detected | Reject with explanation and suggest corrective action. |
...
- [ ] **AC-1:** End-to-end training needs, calendar & cpd tracking workflow operates correctly with ACID integrity.
...
|---|---|---|---|
| `TC-05-01` | Core Training Needs, Calendar & CPD Tracking Workflow | 1. Initiate transaction. 2. Route through approval chain. 3. Execute. 4. Verify audit trail. | Transaction processed, approvals recorded, audit trail complete. |
