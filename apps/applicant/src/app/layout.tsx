...
import './globals.css';
import { ApplicantProviders } from './providers';
...
export default function RootLayout({ children }: { children: React.ReactNode }) {
  return (
    <html lang="en" suppressHydrationWarning className={`${quicksand.variable} ${nunito.variable}`}>
      <body suppressHydrationWarning style={{ margin:0, padding:0, minHeight:'100vh', display:'flex', flexDirection:'column', fontFamily:"var(--font-quicksand,'Quicksand'),var(--font-nunito,'Nunito'),system-ui,sans-serif", background:'#f8fafc', color:'#0f172a' }}>
        ...
        <header style={{ position:'sticky', top:0, zIndex:100, background:'rgba(10,62,80,0.97)', backdropFilter:'blur(12px)', borderBottom:'1px solid rgba(255,255,255,0.1)', boxShadow:'0 2px 16px rgba(0,0,0,0.2)' }}>
          <div style={{ maxWidth:'1200px', margin:'0 auto', padding:'0 2rem', height:'66px', display:'flex', alignItems:'center', justifyContent:'space-between', gap:'1rem' }}>
            ...
            <nav style={{ display:'flex', alignItems:'center', gap:'0.15rem' }}>
              ...
              <a href="http://localhost:3001/status" className="app-nav-link"><FileText style={{ width:'0.85rem', height:'0.85rem' }} /> Track Application</a>
        ...
        <main style={{ flex:1, maxWidth:'1200px', width:'100%', margin:'0 auto', padding:'2rem' }}>
          <ApplicantProviders>{children}</ApplicantProviders>
        </main>
